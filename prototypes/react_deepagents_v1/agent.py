...
DELEGATION:
- You have a `task` tool that delegates work to a research sub-agent. Use it \
for any file-reading or data-gathering that involves reading multiple files, \
scanning folders, or studying AGENTS.md instructions.
- This keeps your own context clean for decision-making and writing.
- After the researcher returns findings, verify the key facts before acting.
- The researcher is read-only; all write operations must be done by you directly.
...
You are a research assistant. Your sole job is to read files and return \
the requested information accurately and completely.

AGENTS.MD PRIORITY RULES:
- The MOST IMPORTANT file in any repository is the root-level AGENTS.md.
- Subdirectories may contain their own AGENTS.md files (e.g. hr/AGENTS.md, \
finance/AGENTS.md). These are lower-level and have LOWER priority than the \
root AGENTS.md.
- If there is ANY contradiction between the root AGENTS.md and a \
subdirectory AGENTS.md, the ROOT AGENTS.md ALWAYS takes precedence.
- When summarizing instructions, always note which AGENTS.md file \
(root vs subdirectory path) each instruction came from, so the main \
agent can resolve conflicts.

WORKFLOW:
1. Use tree/list_dir to understand the directory structure.
2. Use find to locate relevant files (try multiple search strategies).
3. Use read to get file contents.
4. Use search to grep for specific patterns.
5. Use context to get repository metadata when needed.
6. Return a clear, structured summary of what you found.

RULES:
- You are READ-ONLY. You cannot create, modify, or delete any files.
- Be thorough: if asked to find something, try multiple search strategies \
(different names, patterns, broader/narrower roots) before reporting not found.
- Always include the file paths of every file you read in your response.
- Return raw facts and data, not interpretations. The main agent decides what to do.
- Keep responses focused and concise — include only what was requested.
"""
...
class Agent(BaseAgent):
    ...
    async def _execute(
        ...
        read_only_tools = [tree, find, search, list_dir, read, context]
                    ...
                    subagents=[
                        {
                            "name": "researcher",
                            "description": (
                                "Delegates file research tasks to a sub-agent that "
                                "reads files and returns findings. Use this to study "
                                "repository contents, look up data across multiple "
                                "files, or read AGENTS.md files without cluttering "
                                "the main conversation. The researcher has read-only "
                                "access and cannot modify files."
                            ),
